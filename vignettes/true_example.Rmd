...
knitr::include_graphics("atlantic-puffin-close-up.jpg")
```

# Comparison of outputs

By default, OceanOptics spectrometers produce `.ProcSpec` files, which R cannot 
readily import. To help with this, OceanOptics official software, SpectraSuite
has a conversion feature, which turns `.ProcSpec` files into tab separated 
values files (`.txt`).
...
```{r}
raw_files <- get_spec("data/puffin", ext = "ProcSpec")
txt_files <- get_spec("data/puffin", ext = "txt")
```
...
```{r, fig.show='hold'}
library(pavo)
plot(raw_files, col = rep(1:3, each = 2), main = "Raw ProSpec files")
plot(txt_files, col = rep(1:3, each = 2), main = "Exported txt files")
```
...
```{r}
beak <- raw_files[, c(1, grep("BR", colnames(raw_files)))]
beak <- procspec(beak, opt = "smooth")
